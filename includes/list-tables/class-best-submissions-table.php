...
class SpiderRewards_Best_Submissions_Table extends WP_List_Table {

	public function __construct() {
		parent::__construct(
			array(
				'singular' => 'best_submission',
				'plural'   => 'best_submissions',
				'ajax'     => false,
			)
		);
	}
	...
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="best_submission[]" value="%s" />', $item['id'] );
	}
	...
	public function column_status( $item ) {
		...
		if ( $item['status'] !== 'approved' ) {
			$actions['approve'] = sprintf( '<a href="#" class="status-action" data-id="%d" data-status="approved" data-table="best">%s</a>', $item['id'], __( 'Approve', 'spider-rewards' ) );
		}
		if ( $item['status'] !== 'rejected' ) {
			$actions['reject'] = sprintf( '<a href="#" class="status-action" data-id="%d" data-status="rejected" data-table="best">%s</a>', $item['id'], __( 'Reject', 'spider-rewards' ) );
		}
		if ( $item['status'] !== 'pending' ) {
			$actions['pending'] = sprintf( '<a href="#" class="status-action" data-id="%d" data-status="pending" data-table="best">%s</a>', $item['id'], __( 'Set Pending', 'spider-rewards' ) );
		}
